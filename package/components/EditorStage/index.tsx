...
import { getDistance, getImageSize, rotatePoint } from "@/styles/utils";
...
const DeleteArea = styled.div<{ deleteAreaStatus: DeleteAreaStatus }>`
  ${(props) => css`
    ...
    --fill-color: ${props.deleteAreaStatus === "active"
      ? "#ff6650"
      : "#0096ff"};
...
const EditorStage = forwardRef<EditorRefType, EditorPropsType>(
  ({ toolbarBottom }, ref) => {
    const {
      width,
      height,
    ...
    const [deleteAreaStatus, setDeleteAreaStatus] =
      useState<DeleteAreaStatus>("none");
    ...
    const basicScaleRatio = useMemo(() => {
      ...
      const [clipContainWidth] = getImageSize(
        clipRect.width,
        clipRect.height,
        ...containerSize
      );
    ...
    const [dx, dy] = useMemo(() => {
      ...
      const clipCenterX =
        group.x + (clipRect.x + clipRect.width / 2) * basicScaleRatio;

      const clipCenterY =
        group.y + (clipRect.y + clipRect.height / 2) * basicScaleRatio;
    ...
    useEffect(() => {
      deleteAreaRef.current = stage.current?.findOne(
        "#delete-area"
      ) as Konva.Group;
    }, []);
    ...
    return (
      <StageContainer
        ...
        {activeTool === "Words" && (
          <WordInput
            onDone={handleTextAdd}
            onCancel={() => handleSelectTool(null)}
          />
        )}
        {deleteAreaStatus === "none" && (
          <Toolbar onSelect={handleRemoveTransfromer}>{toolbarBottom}</Toolbar>
        )}
      </StageContainer>
    );
  }
