...
import { Point } from "@/styles/types";
...
const ClipStage: ComponentType<ClipStageProps> = ({ onCutDone }) => {
  ...
  const [dx, dy] = useMemo(() => {
    ...
    const clipCenterX =
      group.x + (clipInfo.x + clipInfo.width / 2) * basicScaleRatio;

    const clipCenterY =
      group.y + (clipInfo.y + clipInfo.height / 2) * basicScaleRatio;
  ...
  const handleTransformEnd = () => {
    ...
    currentRectBox.height =
      currentRectBox.height < 0
        ? -currentRectBox.height
        : currentRectBox.height;
    currentRectBox.width =
      currentRectBox.width < 0 ? -currentRectBox.width : currentRectBox.width;
